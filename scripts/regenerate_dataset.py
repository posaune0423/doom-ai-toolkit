...
def preserve_and_renumber_usecase_images(output_dir: str, new_start: int, usecase_start: int, usecase_end: int):
    """
...
Examples:
  ...
  python scripts/regenerate_dataset.py btc --tag "<$BTC>"
    ...
    print(f"Tag: {tag}")
    print("Pattern: Color → Size → Rotation (3 colors × 3 sizes × 5 rotations = 45 images)")
    ...
    if usecase_start is None or usecase_end is None:
        ...
        if detected:
            ...
            print(f"Auto-detected usecase images: {usecase_start:04d}-{usecase_end:04d}")
    ...
    if usecase_start is not None and usecase_end is not None:
        print(f"\nThis will:")
        print(f"  1. Generate new images 0001-{new_generation_end:04d} according to the pattern")
        print(f"  2. Renumber usecase images {usecase_start:04d}-{usecase_end:04d} to {new_usecase_start:04d}-{new_usecase_start + (usecase_end - usecase_start):04d}")
        print(f"  3. Delete old files 0001-{usecase_end:04d} (except logo files)")

        preserve_and_renumber_usecase_images(dataset_dir, new_usecase_start, usecase_start, usecase_end)
    else:
        print(f"\nThis will:")
        print(f"  1. Generate new images 0001-{new_generation_end:04d} according to the pattern")
        print(f"  2. Delete old files (except logo files)")
    ...
    if usecase_start is not None and usecase_end is not None:
        print(f"Total images: {final_end} (0001-{new_generation_end:04d} pattern images + {new_usecase_start:04d}-{final_end:04d} usecase images)")
    else:
